...
import co.amasel.client.common.ApiRequestParams;
...
public class MwsApiResponse {
    private final Logger logger = LoggerFactory.getLogger("MwsApiResponse");
    ...
    public JsonObject asJsonObject(ApiRequestParams params) {
        ...
        if(result != null){
            ...
            logger.info("asJsonObject result json=", out.getJsonObject().encodePrettily());
    ...
    public JsonArray asJsonArray(ApiRequestParams params) {
        ...
        if (resultList != null) {
            ...
            logger.info("asJsonArray result json=", arr.encodePrettily());
            return arr;
        }
        if(tabReport!=null){
            return tabReport;
        ...
        for (CSVRecord line : parser) {
            if(params.asDict){
                ...
                for(Map.Entry<String, String> it: line.toMap().entrySet() ){
                    if (toNumbers && LocaleMisc.isNumber.matcher(it.getValue()).find()) {
                ...
                for (int i = 0; i < line.size(); ++i) {
                    ...
                    if (toNumbers && LocaleMisc.isNumber.matcher(column).find()) {
    ...
    public Object asJsonObjectOrArray(ApiRequestParams params){
        logger.info("asJsonObjectOrArray isArray()=", isArray());
        if(isArray()){
