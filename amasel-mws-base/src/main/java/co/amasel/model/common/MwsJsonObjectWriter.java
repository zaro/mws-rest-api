...
public class MwsJsonObjectWriter implements MwsWriter {
    ...
    JsonObject domNodeToJsonObject(Node node) {
        ...
        if(children!= null) {
            for (int i = 0; i < children.getLength(); ++i) {
                ...
                //putDomNode(newObj, child.getNodeName(), child);
            }
        }
        return newObj;
    }

    void putAppend(JsonObject o, String key, Object newObj){
        if (o.containsKey(key)) {
            JsonArray appendTo = new JsonArray();
            Object current = o.getValue(key);
            if(!(current instanceof JsonArray)){
                // object, move to array
                Object v = o.getValue(key);
                if( v== null){
                    appendTo.addNull();
                } else {
                    appendTo.add(v);
    ...
    void putDomNode(JsonObject o,String key,Node node){
        if( !node.hasChildNodes() && !node.hasAttributes()) {
            ...
            if(tn.isTextNode && tn.text != null) {
                putAppend(o, key, tn.isTextNode ? tn.text : null);
            }
            return ;
        }
        TextNode tn = getNodeText(node);
        if(tn.isTextNode) {
            putAppend(o, key, tn.text);
            return;
        }
        JsonObject newObj = domNodeToJsonObject(node);
        if(newObj.size() > 0 ){
            putAppend(o, key, newObj);
        }
    }
    ...
    void putValue(String key, Object value) {
        ...
        if (value == null) {
            o.putNull( key );
        } else if (value instanceof AmaselMwsObject) {
            beginObject( key );
        ...
        } else if (value instanceof  co.amasel.model.common.ResponseMetadata) {
        } else {
            throw new IllegalArgumentException("Unsupported type "+value.getClass().getName());
        }
    }
